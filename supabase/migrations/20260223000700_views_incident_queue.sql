-- Phase 1.x — Queue views (Incident Queue)
-- One row per INCIDENT work item, with aggregated player info for fast queue rendering.
...
where w.type = 'INCIDENT';

grant select on public.v_incident_queue to authenticated;