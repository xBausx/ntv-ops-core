...
import {
  CLIENT_CREATABLE_INSTALL_STATUSES,
  INSTALL_WORK_STATUSES,
  type InstallCacheState,
  type InstallQueueRow,
  type InstallQueueTableRow,
} from './models/installations.models';
  ...
  template: `
    <section class="space-y-6">
      ...
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="min-w-0">
          <h1 class=\"page-title\">Install Queue</h1>
          <p class="mt-1 max-w-3xl text-sm text-white/60">
            Track, schedule, and verify installations. This replaces the “Today Forward” spreadsheet workflow.
          </p>
        ...
        <div class="p-4 md:p-5">
          <div class="flex items-center justify-between gap-4">
            ...
            <div class="text-xs font-semibold text-white/40 text-right">
              <div>
                Source: <span class="text-white/60">v_install_queue</span>
              </div>
              @if (userEmail()) {
                <div class="mt-1">
...
values ('{{ userId() }}', 'ADMIN');</code></pre>
            ...
            @if (isLoading() && rows().length === 0) {
              ...
              <div class="mt-4 rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <p class="mt-1 text-sm text-white/60">
                  Create a work item (New install) or clear filters.
                </p>
              </div>
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
          <div class="card-modern w-full max-w-xl bg-neutral-950 shadow-2xl">
            <div class="p-5 border-b border-white/10 flex items-start justify-between gap-4">
              <div>
                <div class="text-lg font-extrabold tracking-tight">New install</div>
                <div class="mt-1 text-sm text-white/60">
                  Creates a work item (type INSTALL). Audit events are handled by DB triggers.
                </div>
              </div>
            ...
            <div class="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  class="input-modern w-full"
                  placeholder="e.g. Install player at Site ABC"
                  [value]="createSummary()"
...
export class InstallationsPageComponent {
  ...
  private readonly cacheKey = 'queue:installations:v1';
  ...
  // remount toggle to reset Pantry table internal filters (fixes “empty search” / stale filter state)
  readonly tableMounted = signal<boolean>(true);

  // isLoading = "blocking load" (first load with no cache, or paging)
  readonly isLoading = signal<boolean>(false);

  // isRevalidating = background refresh while showing cached data
  ...
  readonly tableRows = computed<InstallQueueTableRow[]>(() =>
    this.rows().map((r) => ({
      ...r,
  ...
  readonly createStatuses = signal<string[]>([...CLIENT_CREATABLE_INSTALL_STATUSES]);

  readonly columns = signal<any[]>([
    { field: 'status', header: 'Status', visible: true, width: '140px', filter: true, type: 'select', options: [...INSTALL_WORK_STATUSES] },
    ...
    { field: 'scheduled_for', header: 'Scheduled', visible: true, width: '190px', filter: true, filterType: 'text' },
  ...
  private async initLoad(): Promise<void> {
    const cached = this.swr.read<InstallCacheState>(this.cacheKey);
    if (cached) {
  ...
  onClear(): void {
    this.searchText.set('');
    // reset Pantry table's internal filter state too
    this.remountTable();
  }
  ...
  async load(append: boolean, pagesToReload: number = 1): Promise<void> {
    ...
    try {
      ...
      const state = await this.swr.revalidate<InstallCacheState>(this.cacheKey, async () => {
        if (append) {
      ...
      this.swr.write<InstallCacheState>(this.cacheKey, {
        ...state,
        userEmail: this.userEmail(),
        userId: this.userId(),
        userRole: this.userRole(),
  ...
  private async fetchPage(page: number): Promise<{ batch: InstallQueueRow[]; hasMore: boolean }> {
    ...
    const batch = (data ?? []) as InstallQueueRow[];
  ...
  private matches(row: InstallQueueTableRow, q: string): boolean {
